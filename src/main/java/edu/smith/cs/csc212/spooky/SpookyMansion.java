	...
	public SpookyMansion() {
		Place entranceHall = insert(
				Place.create("entranceHall", "You are in the grand entrance hall of a large building.\n"
						+ "The front door is locked. How did you get here?", null));
		...
		Place closet = insert(Place.create("closet",
				"On the wall is scratched a series of letters and a skull icon (" + EMOJI_SKULL + ").\n"
						...
						+ "And there is a hole on the ceiling.\n" + "Where does the hole connect to?", null));
		closet.addExit(new Exit("entranceHall", "There is a brown door."));
		closet.addExit(new SecretExit("basement",
				"Wow, some newspaper covered a hole on the floor. There is a ladder connected to somewhere down there."));

		Place basement = insert(
				Place.create("basement", "You have found the basement of the mansion.\n" + "It is darker down here.\n"
						+ "You get the sense a secret is nearby, but you only see the stairs you came from.", null));
		...
		basement.addExit(new SecretExit("closet",
				"Somthing covered a hole on the ceiling. There is a ladder connected to somewhere up there."));
		basement.addExit(new SecretExit("secretRoom", "There is a door covered by newspaper."));

		Place fallingPit = insert(Place.create("fallingPit", "I don't know what you were thinking...", null));
		fallingPit.addExit(new Exit("labyrinth0", "Keep falling."));

		Place attic = insert(Place.create("attic",
				"Something rustles in the rafters as you enter the attic. Creepy.\n" + "It's big up here.", null));
		...
		attic.addExit(new Exit("closet", "There is a spiral slide connected to somewhere down."));

		Place attic2 = insert(Place.create("attic2", "There's definitely a bat in here somewhere.\n"
				+ "This part of the attic is brighter, so maybe you're safe here.", null));
		...
		Place balcony = insert(Place.create("balcony", "In the day, you can see sunlight from the balcony.", "The balcony is so dark in the night."));
		...
		Place kitchen = insert(
				Place.create("kitchen", "You've found the kitchen. You smell old food and some kind of animal.", "Some mice are stealing food from the kitchen. You hear their noise."));
		...
		Place dumbwaiter = insert(Place.create("dumbwaiter", "You crawl into the dumbwaiter. What are you doing?", null));
		...
		Place secretRoom = insert(Place.create("secretRoom", "You have found the secret room.", null));
		...
		secretRoom.addExit(new SecretExit("basement", "There is a dark hallway with a door at the end."));

		int hallwayDepth = 6;
		int lastHallwayPart = hallwayDepth - 1;
		for (int i = 0; i < hallwayDepth; i++) {
			Place hallwayPart = insert(Place.create("hallway" + i,
					"This is a very long hallway. You have spent " + (i + 1) + " hours in it.", null));
			if (i == 0) {
		...
		Place labyrinth0 = insert(Place.create("labyrinth0", labyrinthDescription, null));
		Place labyrinth1 = insert(Place.create("labyrinth1", labyrinthDescription, null));
		Place labyrinth2 = insert(Place.create("labyrinth2", labyrinthDescription, null));
		Place labyrinth3 = insert(Place.create("labyrinth3", labyrinthDescription, null));
		Place labyrinth4 = insert(Place.create("labyrinth4", labyrinthDescription, null));
		...
		labyrinth3.addExit(new Exit("labyrinth4", "Go West."));

		// solution: East
		labyrinth4.addExit(new Exit("labyrinth0", "Go North."));
		labyrinth4.addExit(new Exit("entranceHall", "Go East."));
		labyrinth4.addExit(new Exit("labyrinth0", "Go South."));
		labyrinth4.addExit(new Exit("labyrinth0", "Go West."));
